...
CREATE TABLE `lands` (
  ...
  `title` VARCHAR(150) NOT NULL COMMENT 'Property / listing display name',
  `title_type` ENUM('TCT', 'OCT', 'Tax_Declaration', 'CLOA') NOT NULL DEFAULT 'TCT',
  `title_number` VARCHAR(100) DEFAULT NULL COMMENT 'e.g., TCT No. 004-2023001234',
  `rod_name` VARCHAR(100) DEFAULT NULL COMMENT 'Registry of Deeds branch',
  `epeb_type` VARCHAR(50) DEFAULT 'CCV' COMMENT 'Electronic Primary Entry Book type (e.g. CCV)',
  `epeb_no` VARCHAR(50) DEFAULT NULL COMMENT 'LRA LOTS transaction reference number',
  `title_document_path` VARCHAR(255) DEFAULT NULL COMMENT 'Certified True Copy or Title scan file path',
  `lra_receipt_path` VARCHAR(255) DEFAULT NULL COMMENT 'LRA official receipt scan file path',
  `is_lra_verified` TINYINT(1) DEFAULT 0,
